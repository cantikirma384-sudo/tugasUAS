...
class Newsx_Core_WXR_Importer extends WP_Importer {
	/**
	 ...
	 * Logger instance.
	 *
	 * @var Newsx_Core_WP_Importer_Logger
	 */
	protected $logger;
	...
	protected function get_reader( $file ) {
		...
		if ( ! $status ) {
			return new WP_Error( 'newsx_core_wxr_importer.cannot_parse', __( 'Could not open the file for parsing', 'news-magazine-x-core' ) );
		}
				...
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						$this->logger->warning( sprintf(
							__( 'This WXR file (version %s) is newer than the importer (version %s) and may not be supported. Please consider updating.', 'news-magazine-x-core' ),
							$this->version,
	...
	public function import( $file ) {
		add_filter( 'newsx_core_import_post_meta_key', array( $this, 'is_valid_meta_key' ) );
	...
	protected function import_start( $file ) {
		if ( ! is_file( $file ) ) {
			return new WP_Error( 'newsx_core_wxr_importer.file_missing', __( 'The file does not exist, please try again.', 'news-magazine-x-core' ) );
		}
		 ...
		 */
		do_action( 'newsx_core_import_start' );
	}
	...
	protected function import_end() {
		 ...
		 */
		do_action( 'newsx_core_import_end' );
	}
	...
	public function set_user_mapping( $mapping ) {
		foreach ( $mapping as $map ) {
			if ( empty( $map['old_slug'] ) || empty( $map['old_id'] ) || empty( $map['new_id'] ) ) {
				$this->logger->warning( __( 'Invalid author mapping', 'news-magazine-x-core' ) );
			...
			switch ( $child->tagName ) {
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						// Bail now
						return new WP_Error(
							'newsx_core_wxr_importer.post.cannot_import_draft',
							__( 'Cannot import auto-draft posts', 'news-magazine-x-core' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		 ...
		 */
		$data = apply_filters( 'newsx_core_wxr_importer.pre_process.post', $data, $meta, $comments, $terms );
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( ! $post_type_object ) {
			$this->logger->warning( sprintf(
				__( 'Failed to import "%s": Invalid post type %s', 'news-magazine-x-core' ),
				$data['post_title'],
				$data['post_type']
			) );
			return false;
		...
		if ( $post_exists ) {
			$this->logger->info( sprintf(
				__( '%s "%s" already exists.', 'news-magazine-x-core' ),
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.process_already_imported.post', $data );
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				$this->logger->notice( sprintf( 
					...
					__( 'Skipping attachment "%s", fetching attachments disabled', 'news-magazine-x-core' ),
					$data['post_title']
				) );
				/**
				 ...
				 */
				do_action( 'newsx_core_wxr_importer.process_skipped.post', $data, $meta );
				return false;
			}
		...
		if ( is_wp_error( $post_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import "%s" (%s)', 'news-magazine-x-core' ),
				$data['post_title'],
				$post_type_object->labels->singular_name
			) );
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.process_failed.post', $post_id, $data, $meta, $comments, $terms );
			return false;
		}
		...
		$this->logger->info( sprintf(
			__( 'Imported "%s" (%s)', 'news-magazine-x-core' ),
			$data['post_title'],
			$post_type_object->labels->singular_name
		) );
		$this->logger->debug( sprintf(
			__( 'Post %d remapped to %d', 'news-magazine-x-core' ),
			$original_id,
			$post_id
		) );
		...
		// Mark items as NEWSX Imported
		add_post_meta( $post_id, '_newsx_demo_import_item', true );
		 ...
		 * @param array $terms Raw term data, already processed.
		 */
		do_action( 'newsx_core_wxr_importer.processed.post', $post_id, $data, $meta, $comments, $terms );
	}
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		if ( ! $info ) {
			return new WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'news-magazine-x-core' ) );
		}
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		foreach ( $meta as $meta_item ) {
			 ...
			 */
			$meta_item = apply_filters( 'newsx_core_wxr_importer.pre_process.post_meta', $meta_item, $post_id );
			if ( empty( $meta_item ) ) {
				return false;
			}

			$key = apply_filters( 'newsx_core_import_post_meta_key', $meta_item['key'], $post_id, $post );
			$value = false;
			...
			if ( $key ) {
				...
				do_action( 'newsx_core_import_post_meta', $post_id, $key, $value );
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		foreach ( $comments as $key => $comment ) {
			 ...
			 */
			$comment = apply_filters( 'newsx_core_wxr_importer.pre_process.comment', $comment, $post_id );
			if ( empty( $comment ) ) {
				return false;
			}
			...
			if ( $post_exists ) {
				...
				if ( $existing ) {
					 ...
					 */
					do_action( 'newsx_core_wxr_importer.process_already_imported.comment', $comment );
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.processed.comment', $comment_id, $comment, $meta, $post_id );
	...
	protected function process_author( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'newsx_core_wxr_importer.pre_process.user', $data, $meta );
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( is_wp_error( $user_id ) ) {
			$this->logger->error( sprintf(
				__( 'Failed to import user "%s"', 'news-magazine-x-core' ),
				$userdata['user_login']
			) );
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.process_failed.user', $user_id, $userdata );
			return false;
		}
		...
		$this->logger->info( sprintf(
			__( 'Imported user "%s"', 'news-magazine-x-core' ),
			$userdata['user_login']
		) );
		$this->logger->debug( sprintf(
			__( 'User %d remapped to %d', 'news-magazine-x-core' ),
			$original_id,
			$user_id
		) );
		 ...
		 * @param array $userdata Raw data imported for the user.
		 */
		do_action( 'newsx_core_wxr_importer.processed.user', $user_id, $userdata );
	}
	...
	protected function process_term( $data, $meta ) {
		 ...
		 * @param array $meta Meta data.
		 */
		$data = apply_filters( 'newsx_core_wxr_importer.pre_process.term', $data, $meta );
		if ( empty( $data ) ) {
			return false;
		}
		...
		if ( $existing ) {
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.process_already_imported.term', $data );
		...
		if ( is_wp_error( $result ) ) {
			 ...
			 */
			do_action( 'newsx_core_wxr_importer.process_failed.term', $result, $data, $meta );
			return false;
		}
		...
		$this->logger->debug( sprintf(
			__( 'Term %d remapped to %d', 'news-magazine-x-core' ),
			$original_id,
			$term_id
		) );
		...
		// Mark items as NEWSX Imported
		add_post_meta( $term_id, '_newsx_demo_import_item', true );
		 ...
		 * @param array $data Raw data imported for the term.
		 */
		do_action( 'newsx_core_wxr_importer.processed.term', $term_id, $data );
	}
	...
	protected function fetch_remote_file( $url, $post ) {
		...
		if ( $code !== 200 ) {
			...
			return new WP_Error(
				'import_file_error',
				sprintf(
					__( 'Remote server returned %1$d %2$s for %3$s', 'news-magazine-x-core' ),
					$code,
		...
		if ( isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'news-magazine-x-core' ) );
		}

		if ( 0 === $filesize ) {
			unlink( $upload['file'] );
			return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'news-magazine-x-core' ) );
		}

		$max_size = (int) $this->max_attachment_size();
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			unlink( $upload['file'] );
			$message = sprintf( __( 'Remote file is too large, limit is %s', 'news-magazine-x-core' ), size_format( $max_size ) );
	...
	protected function post_process_posts( $todo ) {
		foreach ( $todo as $post_id => $_ ) {
			$this->logger->debug( sprintf(
				...
				__( 'Running post-processing for post %d', 'news-magazine-x-core' ),
				$post_id
			) );
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['post'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the post parent for "%s" (post #%d)', 'news-magazine-x-core' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with parent %d, but could not be found', 'news-magazine-x-core' ),
						$post_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_slug ) ) {
				...
				if ( isset( $this->mapping['user_slug'][ $author_slug ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for "%s" (post #%d)', 'news-magazine-x-core' ),
						get_the_title( $post_id ),
						$post_id
					) );
					$this->logger->debug( sprintf(
						__( 'Post %d was imported with author "%s", but could not be found', 'news-magazine-x-core' ),
						$post_id,
						$author_slug
					) );
				}
			...
			if ( empty( $data ) ) {
				$this->logger->debug( sprintf(
					__( 'Post %d was marked for post-processing, but none was required.', 'news-magazine-x-core' ),
					$post_id
				) );
				continue;
			}
			...
			if ( is_wp_error( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update "%s" (post #%d) with mapped data', 'news-magazine-x-core' ),
					get_the_title( $post_id ),
					$post_id
				) );
				$this->logger->debug( $result->get_error_message() );
	...
	protected function post_process_menu_item( $post_id ) {
		...
		if ( ! empty( $menu_object ) ) {
			...
			$this->logger->warning( sprintf(
				__( 'Could not find the menu object for "%s" (post #%d)', 'news-magazine-x-core' ),
				get_the_title( $post_id ),
				$post_id
			) );
			$this->logger->debug( sprintf(
				__( 'Post %d was imported with object "%d" of type "%s", but could not be found', 'news-magazine-x-core' ),
				$post_id,
				$menu_object_id,
				$menu_item_type
			) );
		...
		foreach ( $todo as $comment_id => $_ ) {
			...
			if ( ! empty( $parent_id ) ) {
				...
				if ( isset( $this->mapping['comment'][ $parent_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the comment parent for comment #%d', 'news-magazine-x-core' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with parent %d, but could not be found', 'news-magazine-x-core' ),
						$comment_id,
						$parent_id
					) );
				}
			...
			if ( ! empty( $author_id ) ) {
				...
				if ( isset( $this->mapping['user'][ $author_id ] ) ) {
					...
					$this->logger->warning( sprintf(
						__( 'Could not find the author for comment #%d', 'news-magazine-x-core' ),
						$comment_id
					) );
					$this->logger->debug( sprintf(
						__( 'Comment %d was imported with author %d, but could not be found', 'news-magazine-x-core' ),
						$comment_id,
						$author_id
					) );
				}
			...
			if ( empty( $result ) ) {
				$this->logger->warning( sprintf(
					__( 'Could not update comment #%d with mapped data', 'news-magazine-x-core' ),
					$comment_id
				) );
				continue;
			}
